...
### Подготовка слов
Для начала уберем пунктуацию и переведем все буквы в строчные  отдельной программой (папка tools prepare_words.cpp).
...
| Функция       | До оптимизации (инструкции) | После оптимизации (инструкции) | Ускорение |
|---------------|-----------------------------|----------------------------------|-----------|
| `search_word_table()` |  ~15.9 млрд              | ~17.4 млрд                      | **0.91x** |
...
| Функция       | До оптимизации (инструкции) | После оптимизации (инструкции) | Ускорение |
|---------------|-----------------------------|----------------------------------|-----------|
| `search_word_table()` |  ~15.9 млрд              | ~17.9 млрд                      | **0.88x** |
...
| Этап | Версия функции | Такты `Self` | Время | Ускорение | Комментарий |
|------|------------------|----------------|---------|------------|-------------|
| 1    | `hash()` → CRC32 intrinsic | ~17.4 млрд → ~2.8 млрд | 1.23 → 0.94 с | **1.31x** | +6.21x по самой хэш-функции |
| 2    | `strcmp()` → AVX2 intrinsic | ~15.8 млрд → ~13.2 млрд | 0.94 → 0.73 с | **1.29x** | +
| 3    | `search_word_table()` → AVX2 | ~30.4 млрд → ~15.9 млрд | 0.73 → 0.70 с | **1.04x** | Достигнуто 4% ускорение |
| 4    | `search_word_table()` → inline ASM | ~15.9 млрд → ~17.4 млрд | 0.70 → 0.69 с | **~1.01x** | Не дало значительного выигрыша |
| 5    | `hash()` → чистый ASM | ~2.8 млрд → ~1.9 млрд | 0.70 → 0.83 с | **0.84x** | Замедлило работу из-за отсутствия оптимизации контекста |

---

# > ✨ **Итоги:**  
> - Наилучший результат: **AVX2 интринсики** в `search_word_table()`  
> - Коэффициент улучшения на момент достижения желаемого прироста равен  195.
